...
export function setupUI(app: HTMLElement, callbacks: ControlsCallbacks): {
  ...
  app.innerHTML = `
    ...
    <div class="controls">
      <div class="control-group">
        <div class="control-label"><span>mood</span></div>
        <div class="mood-selector" id="moodSelector">
          ${MOODS.map(m => `<button class="mood-btn${m === 'downtempo' ? ' active' : ''}" data-mood="${m}">${m}</button>`).join('')}
        </div>
      </div>
      <div class="control-group">
        <div class="control-label"><span>density</span><span id="densityVal">50%</span></div>
        <input type="range" id="density" min="0" max="100" value="50" />
      </div>
      <div class="control-group">
        <div class="control-label"><span>brightness</span><span id="brightnessVal">40%</span></div>
        <input type="range" id="brightness" min="0" max="100" value="40" />
      </div>
      <div class="control-group">
        <div class="control-label"><span>spaciousness</span><span id="spaciousnessVal">70%</span></div>
        <input type="range" id="spaciousness" min="0" max="100" value="70" />
      </div>
    </div>
    <div class="state-display" id="stateDisplay">
      <div class="state-row"><span class="state-key">scale</span><span class="state-value" id="scaleDisplay">C dorian</span></div>
      <div class="state-row"><span class="state-key">chord</span><span class="state-value" id="chordDisplay">Cm7</span></div>
      <div class="state-row"><span class="state-key">elapsed</span><span class="state-value" id="elapsedDisplay">0:00</span></div>
    </div>
  `;
  ...
  function updateState(state: GenerativeState): void {
    ...
    if (scaleEl) scaleEl.textContent = `${state.scale.root} ${state.scale.type}`;
